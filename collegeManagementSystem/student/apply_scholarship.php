...
<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $semester=$target_dir . basename($_FILES['sem_certificate']['name']);
        ...
        if(move_uploaded_file($_FILES['income_certificate']['tmp_name'],$income)){
            if(move_uploaded_file($_FILES['caste_certificate']['tmp_name'],$caste)){
                if(move_uploaded_file($_FILES['photo']['tmp_name'],$photo)){
                    if(move_uploaded_file($_FILES['sem_certificate']['tmp_name'],$semester)){
                        $query=mysqli_query($conn,"insert into scholarship_ug(student,name,email,phone,dob,gender,marital_status,address,father_name,father_occupation,mother_name,mother_occupation,department,scholarship_type,income_cert,caste_cert,photo,sem_cert)values('$std','$name','$email','$phone','$dob','$gender','$marital_status','$address','$fn','$fo','$mn','$mo','$dept','$sch_type','$income','$caste','$photo','$semester')");
                        if($query){
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-field">
                            <div class="file-upload-container" style="max-width: 400px; margin: 0 auto;">
                                <p><i class="fas fa-image"></i> Semester Certificate (PDF)
                                <span class="file-name" id="name_sem_certificate">No file chosen</span>
                                </p>
                                <input type="file" id="sem_certificate" name="sem_certificate" required>
                                <label for="photo">Click here to upload</label>
                            </div>
